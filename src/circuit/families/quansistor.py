...
class QuansistorBrickwork:
    ...
    def make_spec(self, n_qubits: int, n_layers: int, d: int, seed: int | None, *, topology: str = "loop", **kwargs: Any) -> CircuitSpec:
        return CircuitSpec(n_qubits=n_qubits, n_layers=n_layers, d=d, global_seed=seed, family=self.name, topology=topology, params={})
    ...
    def gates(self, spec: CircuitSpec) -> Iterable[GateSpec]:
        for layer in range(spec.n_layers):
            ...
            for idx, (q0, q1, q2, q3) in enumerate(blocks):
                ...
                s = gate_seed(spec.seed, layer=layer, slot=idx, wires=(q0,q1,q2,q3), kind="quansistor_block")
                yield GateSpec(
            ...
            for j, (a,b) in enumerate(pairs):
                s = gate_seed(spec.seed, layer=layer, slot=1000+j, wires=(a,b), kind="quansistor2")
                yield GateSpec(
                    kind="quansistor2",
                    wires=(a,b),
                    d=spec.d,
